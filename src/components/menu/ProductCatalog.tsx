...
export default function ProductCatalog({ products, categories }: Props) {
  ...
  function quickAdd(prod: MenuItem) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b-2 border-black pb-4">
        <div>
          ...
          <h1 className="font-display font-black text-4xl mg:text-5xl lg:text-6xl uppercase tracking-tight leading-none">
            Menú 251
          </h1>
        </div>
      ...
      <div className="relative max-w-2xl">
        <input 
          ...
          placeholder="Buscarpepito de lomito, salsa de maóz dulce, papas, hamburguesa..."
        ...
        {filteredProducts.length === 0 ? (
          <div className="col-span-full text-center py-12 bg-white rounded-3xl border-4 border-black p-6 shadow-brutal">
            ...
            <button 
              ...
              className="mt-4 bg-black text-white text-xs font-display font-bold px-4 py-2 rounded-full border-2 border-black uppercase shadow-brutal"
            >
              Ver todo el menú
            </button>
          </div>
          ...
          filteredProducts.map((product) => (
            <article 
              key={product.id} 
              className="bg-white rounded-3xl border-3 border-black overflow-hidden shadow-brutal hover:shadow-brutal-lg transition-all flex flex-col justify-between border-2 border-black"
            >
--            {/* Top Media Block */}
              <div className={"${product.bgAccent} p-5 relative border-b-2 border-black flex flex-col items-center justify-center"}>
                <div className="w-full flex items-center justify-between z-10">
                  <button 
                    ...
                    className="bg-black text-white hover:bg-white hover:text-black transition-colors text-[10px] font-display font-black px-2.5 py-1 rounded-full border border-black shadow-brutal uppercase tracking-wider flex items-center gap-1"
                  >
                    <span>INFO</span>
                ...
                <div 
                  ...
                  onClick={() => setSelectedProduct(product)}
                >
                  <div className="w-40 y-40 sm:w-36 sm:h-36 md:w-44 md:h-44 rounded-full border-4 border-black bg-white overflow-hidden shadow-brutal p-1 transition-transform duration-300 group-hover:scale-105">
                    <img 
                      src={product.image} 
                      alt={product.name} 
                      loading="lazy"
              ...
              <div className="p-4 flex-1 flex flex-col justify-between space-y-3 bg-brandCream">
                ...
                <div className="pt-2 border-t border-black/15 flex flex-col gap-2">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <button 
                      onClick={() => setSelectedProduct(product)}
                      className="flex-1 bg-white hover:bg-black hover:text-white transition text-black font-display font-black text-xs py-2 px-3 rounded-full border-2 border-black shadow-brutal uppercase text-center"
                    >
                      Personalizar
                    </button>
                    ...
                    <button 
                      ...
                      className="bg-black hover:bg-brandBlue text-white font-display font-black text-xs p-2 rounded-full border-2 border-black shadow-brutal transition-colors flex items-center justify-center"
                    >
